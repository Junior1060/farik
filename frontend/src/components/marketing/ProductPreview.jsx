      ...
      <div className="flex">
        <div className="hidden md:flex w-44 flex-col border-r border-slate-100 py-4 px-3 gap-0.5 bg-white">
          <div className="flex items-center gap-2 px-2 mb-4">
            <span className="w-6 h-6 rounded-md bg-indigo-600 flex items-center justify-center"><Building2 size={12} className="text-white" /></span>
            <span className="text-sm font-semibold text-slate-900">Farik</span>
          </div>
          {NAV.map(({ icon: Icon, label, active }) => (
            <div key={label} className={`flex items-center gap-2.5 px-2.5 py-2 rounded-lg text-xs font-medium ${active ? 'bg-indigo-50 text-indigo-700' : 'text-slate-500'}`}>
        ...
        <div className="flex-1 min-w-0 bg-surface-50 p-4 sm:p-5 space-y-4">
          <div className="flex items-center justify-between">
            ...
            <span className="h-7 w-24 rounded-lg bg-indigo-600" />
          </div>
