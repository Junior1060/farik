...
import {
  previewRows, uploadDocuments, confirmImport, aiImport, uploadSpreadsheet, downloadTemplate,
...
function UploadStep({ onParsed }) {
  ...
  const isSpreadsheet = !!selectedFile && /\.(xlsx|xls|csv)$/i.test(selectedFile.name);

  // Non-AI path for files laid out like the downloadable template. It keeps
  // import usable when the AI service is down — which is exactly when the
  // AI error message suggests using the template.
  const handleTemplateSubmit = async () => {
    if (!isSpreadsheet || loading) return;
    setLoading(true);
    setError('');
    try {
      ...
      onParsed(data.rows, null);
    } catch (err) {
      setError(err?.response?.data?.error || 'Farik could not read that spreadsheet. Check that it matches the template and try again.');
    } finally {
      setLoading(false);
    }
  };
      ...
      {showPaste ? (
        ...
        <>
          ...
          <div className="flex flex-wrap items-center gap-x-4 gap-y-1">
            <button type="button" onClick={() => setShowPaste(true)} className="text-xs text-slate-500 hover:text-slate-700 inline-flex items-center gap-1.5">
              <Type size={13} /> or paste your data as text instead
            </button>
            <button type="button" onClick={downloadTemplate} className="text-xs text-slate-500 hover:text-slate-700 inline-flex items-center gap-1.5">
              <Download size={13} /> Download the spreadsheet template
            </button>
          </div>
        </>
      )}
      ...
      <div className="flex flex-col-reverse sm:flex-row sm:items-center sm:justify-between gap-3">
        {isSpreadsheet ? (
          <button
            ...
            className="text-xs text-slate-500 hover:text-slate-700 underline-offset-4 hover:underline text-left"
          >
            Read it as a template spreadsheet instead (no AI)
          </button>
        ) : <span />}
        <button
          type="button"
...
/**
 * The import flow: upload → review → documents → confirm → done. Rendered by the
 * Import page inside the app shell and by onboarding outside it. "Import more"
 * resets React state instead of reloading the browser.
 */
export default function ImportWizard() {
  ...
  return (
    <div className="card">
      ...
      {step === 5 && <SuccessStep result={result} onReset={reset} />}
    </div>
  );
}
