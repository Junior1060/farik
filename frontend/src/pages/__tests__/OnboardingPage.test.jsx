...
const { mockCreateProperty, mockCreateTenant } = vi.hoisted(() => ({
...
function fillProperty() {
  ...
  fireEvent.change(screen.getByLabelText('Province or state'), { target: { value: 'SK' } });
  fireEvent.change(screen.getByLabelText('Postal code'), { target: { value: 'S4S 4H4' } });
  fireEvent.change(screen.getByLabelText('Property type'), { target: { value: 'Duplex or triplex' } });
...
describe('OnboardingPage', () => {
  it('welcomes the landlord by name and offers import or manual setup, never trapping them', () => {
    renderOnboarding();
    expect(screen.getByRole('heading', { level: 1 })).toHaveTextContent('Welcome to Farik, Jordan');
    expect(screen.getByText(/enter everything manually or import what you already have/i)).toBeInTheDocument();
  ...
  it('routes the import choice into the real import wizard', async () => {
    renderOnboarding();
    ...
    expect(screen.getByText(/leases, spreadsheets, tenant lists, rent rolls, or property documents/i)).toBeInTheDocument();
    for (const f of ['PDF', 'Excel', 'CSV', 'Word']) expect(screen.getByText(f)).toBeInTheDocument();
  ...
  it('creates the property with a unit count, then moves on to tenants', async () => {
    ...
    expect(mockCreateProperty).toHaveBeenCalledWith({
      ...
      propertyType: 'Duplex or triplex', unitCount: 2,
    });
