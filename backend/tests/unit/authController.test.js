...
describe('register — self-serve landlord signup', () => {
  it('creates a landlord from a single full-name field and returns a usable token', async () => {
  ...
  it('normalises the email to lower case and trims it before storing', async () => {
    const { req, res, next } = mockReqRes(LANDLORD_SIGNUP);
    await authController.register(req, res, next);
  ...
  it('rejects an existing email with a 409 and a human-readable message', async () => {
    mockPrisma.user.findUnique.mockResolvedValue({ id: 'user-9', email: 'jordan@example.com', role: 'LANDLORD', invitePending: false });
    const { req, res, next } = mockReqRes(LANDLORD_SIGNUP);
    await authController.register(req, res, next);
  ...
  it('treats an email that differs only by case as the same account', async () => {
    mockPrisma.user.findUnique.mockResolvedValue(null);
    mockPrisma.user.findFirst.mockResolvedValue({ id: 'user-9', email: 'Jordan@Example.com', role: 'LANDLORD', invitePending: false });
    const { req, res, next } = mockReqRes(LANDLORD_SIGNUP);
    await authController.register(req, res, next);
...
describe('register — tenant claiming an account a landlord created for them', () => {
  const invited = {
    id: 'user-7',
  ...
  it('lets the tenant set a real password and keeps the profile the landlord linked', async () => {
    ...
    expect(data.invitePending).toBe(false);
  ...
  it('does not let a landlord signup claim a pending tenant account', async () => {
    mockPrisma.user.findUnique.mockResolvedValue(invited);
  ...
  it('does not let anyone claim a tenant account that was registered normally', async () => {
    mockPrisma.user.findUnique.mockResolvedValue({ ...invited, invitePending: false });
    const { req, res, next } = mockReqRes({ fullName: 'Alice Morgan', email: 'alice@example.com', password: 'a-real-password', role: 'TENANT' });
    await authController.register(req, res, next);
...
describe('login', () => {
  ...
  const user = {
    id: 'user-1', email: 'demo@farik.ca', role: 'LANDLORD', password: hashed,
