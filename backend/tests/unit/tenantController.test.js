...
describe('tenantController.create — landlord adds a tenant and their first lease', () => {
  ...
  it('creates a claimable tenant account, the profile, and an active lease in one call', async () => {
    ...
    expect(userData.invitePending).toBe(true);
    ...
    expect(res.body.tenant).toMatchObject({ id: 'tp-1', firstName: 'Alice', user: { email: 'alice@example.com' } });
  ...
  it('links an existing tenant account instead of creating a duplicate', async () => {
    mockPrisma.user.findFirst.mockResolvedValue({
      id: 'user-5', email: 'alice@example.com', role: 'TENANT',
