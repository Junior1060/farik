...
const lookupByEmail = async (req, res, next) => {
  try {
    ...
    if (!user || !user.tenantProfile) {
      return res.status(404).json({ error: 'No tenant account found with this email. Add them from the Tenants page, or ask them to create a tenant account at /signup/tenant first.' });
    }
    ...
    const { phone: _phone, ...tenantProfileSafe } = user.tenantProfile;
...
// email yet, the account is created with a random placeholder password and
// invitePending=true so the tenant's own sign-up later claims it.
...
const create = async (req, res, next) => {
  try {
    ...
    res.status(201).json({ tenant: { ...tenant, user: { email: user.email } }, lease });
  } catch (err) {
    next(err);
  }
};
